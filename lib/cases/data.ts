...
export const ALLCASES: Case[] = [
  {
    ...
    links: [{ label: "GitHub", url: "https://github.com/My-Golden-Kids/MGK" }],

    contributions: [
      {
        roleId: "frontend",
        summary: "인증 구조 설계 및 Reverse Proxy 재설계, STT 보정 로직 구현",
        team: { total: 4, myRole: "프론트엔드" },
      },
      {
        roleId: "backend",
        summary: "Spring Security 인증 흐름 검증, Nginx 헤더 전달 설정",
        ...
        features: [
          ...
          },
          {
            ...
            block: {
              ...
              width: 1920,
              height: 1080,
              caption: "Reverse Proxy 기반으로 재구성된 최종 인증 아키텍처.",
            },
          },
        ],
      },

      /* ── 음성 인식 보정 (split layout) ── */
      {
        id: "stt-correction",
        heading: "음성 인식 보정",
        roles: ["frontend"],
        ...
        blocks: [
          {
            ...
            code: `const CORRECTION_MAP: Record<string, string> = {
  "밤토끼": "밥 두끼",
  "새끼": "세끼",
  "뱁": "밥",
  "배기": "백이",
...
/** 경음 정규화: ㄲ→ㄱ, ㄸ→ㄷ, ㅃ→ㅂ, ㅆ→ㅅ, ㅉ→ㅈ */
function normalizeHardConsonants(text: string) {
  return text
    .replace(/ㄲ/g, "ㄱ")
    .replace(/ㄸ/g, "ㄷ")
    .replace(/ㅃ/g, "ㅂ")
...
export function correctSTT(raw: string): string {
...

  {
    ...
    contributions: [
      ...
      {
        roleId: "backend",
        summary: "Account 중심 데이터 모델 재설계, 반정규화 전략 적용",
        team: { total: 4, myRole: "백엔드" },
      },
    ],
...

  {
    ...
    links: [{ label: "GitHub", url: "https://github.com/Seon-U/blogging.git" }],
        ...
        blocks: [
          {
            ...
            width: 1920,
            height: 1080,
            caption: "StoryBook Select 컴포넌트 활용 예시",
          },
        ],
      },
    ],
...

  {
    ...
    contributions: [
      ...
      {
        roleId: "planner",
        summary: "북한이탈주민 대상 사용자 인터뷰 기반 학습 흐름 기획",
        ...
        blocks: [
          {
            ...
            width: 1212,
            height: 1158,
            caption: "녹음 및 재생 흐름을 담당하는 Audio Manager 구조.",
          },
        ],
      },
